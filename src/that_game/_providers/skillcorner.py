...
def _skillcorner_events_add_type_name(df: pl.DataFrame) -> pl.DataFrame:
    ...
    """
    target_cols = ["event_type", "event_subtype", "start_type", "end_type"]

    return df.with_columns(
        pl.concat_list([pl.col(col) for col in target_cols])
...
skillcorner = Provider(
    data_type="csv",
    preprocess=_skillcorner_events_add_type_name,
    field_map=FieldMap(
        id_="event_id",
        type_="type_name",
    ),
